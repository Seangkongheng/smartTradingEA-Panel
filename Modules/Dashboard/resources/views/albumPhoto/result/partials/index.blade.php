...
<div class="title-table mt-5 flex flex-col md:flex-row  items-center justify-between gap-4">
    ...
    <div class="flex-1 flex justify-between w-full md:min-w-[160px]">
        <h1 class="text-xl font-semibold text-gray-800 kantumruy-pro m-0 p-0">
         ...
         <a href="{{ route('admin.result-photos.create') }}" class="flex md:hidden items-center gap-2 px-4 py-2.5 rounded-lg border border-green-600 bg-white hover:bg-green-50 transition-colors">
    ...
    <div class="flex items-center justify-start  gap-3 flex-1 w-full md:max-w-[800px]">
        ...
        {{--  Noted : Add Button   --}}
        <a href="{{ route('admin.result-photos.create') }}" class="hidden md:flex items-center gap-2 px-4 py-2.5 rounded-lg border border-green-600 bg-white hover:bg-green-50 transition-colors">
            <svg class="w-5 h-5 text-green-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"/>
            </svg>
            <span class="kantumruy-pro font-medium text-green-600">Add New</span>
    ...
    <div class="main-full-content w-full">
        <div class="table-content w-full">
            <div class="relative overflow-x-auto sm:rounded-lg">
               @include('dashboard::albumPhoto.result.partials.tableInformation.productTable')
            </div>
        </div>
    </div>
</div>

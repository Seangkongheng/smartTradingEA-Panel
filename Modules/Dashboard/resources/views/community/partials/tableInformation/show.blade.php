              ...
              <div class="space-y-2">
                  <div class="flex items-center gap-3">
                      <h1 class="text-3xl font-extrabold text-slate-900 tracking-tight">
                          {{ $community->title ?? 'Untitled Subscription' }}
                      </h1>

                  </div>
                  <p class="text-lg text-slate-600 max-w-2xl leading-relaxed">
                      {{ $community->description ?? 'No detailed description provided for this record.' }}
                  </p>
              </div>
              <span
                  class="inline-flex items-center gap-2 px-3 py-1 rounded-full text-xs font-medium
                {{ $community->is_public ? 'bg-green-500/10 text-green-400' : 'bg-red-500/10 text-red-400' }}">
                  <span
                      class="w-2 h-2 rounded-full
                        {{ $community->is_public ? 'bg-green-400' : 'bg-red-400' }}">
                  </span>
          ...
          <div class="lg:col-span-2 space-y-6">
              <div class="bg-white border border-slate-200 rounded-3xl p-6 shadow-sm">
                  <div class="flex items-center justify-between mb-6">
                      <h2 class="text-xl font-bold text-slate-900">Community Gallery</h2>
                      <span class="text-sm text-slate-500 font-medium">Total Files:
                          {{ count(json_decode($community->file ?? '[]', true)) }}</span>
                  </div>

                  @php $files = json_decode($community->file, true); @endphp
              ...
              <div class="bg-white border border-slate-200 rounded-3xl shadow-sm overflow-hidden">
                  ...
                  <div class="p-6 space-y-5">
                      ...
                      <div class="flex flex-col gap-1">
                          ...
                          <span class="font-semibold text-slate-800">{{ $community->created_at->format('M d, Y') }}</span>
                          <span class="text-xs text-slate-500">{{ $community->created_at->format('h:i A') }}</span>
                      </div>

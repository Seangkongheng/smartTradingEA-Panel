<div class="bg-white w-full p-8 rounded-2xl" style="box-shadow: rgba(17, 17, 26, 0.05) 0px 2px 8px, rgba(17, 17, 26, 0.05) 0px 0px 32px;">
             ...
             <h1 class="m-0 p-0 text-lg"><i class="fas fa-graduation-cap text-yellow-500 mr-2"></i>
                    <span class="kantumruy-pro text-lg"> សិទ្ធ </span>
            </h1>
    </div><br>
    <form method="POST" action="{{ isset($permissionEdit->id)? route('admin.permission.update',$permissionEdit->id) : route('admin.permission.store') }}" enctype="multipart/form-data">
        @csrf
        @if(isset($permissionEdit->id))
            @method("PUT")
        @endif
        <div class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-12 gap-4 items-start">
                ...
                <div class="md:col-span-9">
                    <input type="text"  name="name"  value="{{ old('name',isset($permissionEdit->id)?$permissionEdit->name : "") }}" class="px-6 py-3.5 text-black bg-gray-100 w-full rounded-xl outline-none focus:ring-1  focus:ring-green-500 focus:border-green-500" placeholder="Ex. index,.create ..." required>
                    @error('name')
            ...
            <div class="mt-8 flex justify-end">
                <button type="submit" class="inter px-5 py-2 p-8  backdrop-blur-lg text-white bg-green-600 kantumruy-pro rounded-lg items-center gap-0.5 inline-flex border border-white/15 hover:bg-green-600 transition-all duration-300 ease-in-out"
                    id="submit-button"> <span class="kantumruy-pro">{{ isset($permissionEdit->id)?  "កែប្រែ": "រក្សាទុក" }}</span> <svg xmlns="http://www.w3.org/2000/svg" height="20px" viewBox="0 -960 960 960" width="20px" class="fill-current">

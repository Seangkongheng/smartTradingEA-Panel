...
<div class="title-table mt-5 flex flex-col md:flex-row  items-center justify-between gap-4">
    ...
    <div class="flex-1 flex justify-between w-full md:min-w-[160px]">
        <h1 class="text-xl font-semibold text-gray-800 kantumruy-pro m-0 p-0">
            <span class="text-green-600">សាស្រ្តាចារ្យ</span></span>
    ...
    <div class="flex items-center justify-start  gap-3 flex-1 w-full md:max-w-[800px]">
        <div class="flex-1 md:min-w-[160px]">
            <form action="" method="POST">
                <div class="relative">
                    <input type="text" id="search" name="search" placeholder="ស្វែងរក..."
        ...
        <div class="relative sm:w-[160px]">
            <select name="status" disabled class="w-full px-2 md:px-4 kantumruy-pro  pr-8 py-2.5 rounded-lg border border-gray-200 bg-gray-50 focus:bg-white focus:ring-1 focus:ring-green-500 focus:outline-none focus:border-green-500 text-gray-700 appearance-none transition-all">
                <option value="1" selected>មុខវិទ្យា</option>
        ...
        <!-- Add Button -->
        <a href="{{ route('admin.platform.create') }}" class="hidden md:flex items-center gap-2 px-4 py-2.5 rounded-lg border border-green-600 bg-white hover:bg-green-50 transition-colors">
            <svg class="w-5 h-5 text-green-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"/>
            </svg>
            <span class="kantumruy-pro font-medium text-green-600">បន្ថែម</span>
    ...
    <div class="main-full-content w-full">
        <div class="table-content w-full">
            <div class="relative overflow-x-auto sm:rounded-lg">
                @include('dashboard::platform.partials.tableInformation.platformTable')
            </div>
        </div>
    </div>
</div>
{{ $professors->onEachSide(1)->links() }}

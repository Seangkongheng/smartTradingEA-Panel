<div class="main-content w-full">
    <form action="{{ isset($rewardEdit) ? route('admin.rewards.update', $rewardEdit->id) : route('admin.rewards.store') }}"
        ...
        <div class="lg:col-start-1 lg:col-end-13 rounded-2xl bg-[#131d41] p-8">
            ...
            <div class="grid md:grid-cols-12 gap-4 mb-6">
                ...
                <div class="md:col-span-9">
                    <input type="text" required placeholder="Enter title.." name="title"
                        value="{{ old('title', $rewardEdit->title ?? '') }}" class="w-full px-4 py-3 rounded-lg text-black"
                        required>
                </div>
            </div>
            ...
            <div class="grid md:grid-cols-12 gap-4 mb-6">
                ...
                <div class="md:col-span-9">
                    <textarea name="description" placeholder="Enter description.." class="w-full px-4 py-3 rounded-lg text-black"
            ...
            <div id="user-select-section" class="grid md:grid-cols-12 gap-4 mb-6">
                ...
                <div class="md:col-span-9 bg-gray-800 p-4 rounded-lg max-h-64 overflow-y-auto">
                            ...
                            @foreach ($rowChunks as $chunk)
                                <div>
                                    @foreach ($chunk as $user)
                                        <label class="flex items-center space-x-2 text-white mb-2 cursor-pointer">
                ...
                <div class="md:col-span-9 flex gap-6">
                    ...
                    <label class="flex items-center gap-2">
                        <input type="radio" name="is_public" value="1"
                            {{ old('is_public', $rewardEdit->is_public ?? 1) == 1 ? 'checked' : '' }}>
                        <span class="text-white">Public</span>
                    </label>

                    <label class="flex items-center gap-2">
                        <input type="radio" name="is_public" value="0"
                            {{ old('is_public', $rewardEdit->is_public ?? 1) == 0 ? 'checked' : '' }}>

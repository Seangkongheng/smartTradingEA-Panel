    ...
    <tbody class="divide-y divide-gray-700">
        @forelse ($rewards as $i => $reward)
            <tr class="hover:bg-gray-800/40 transition">
                ...
                {{ $reward->is_public ? 'bg-green-500/10 text-green-400' : 'bg-red-500/10 text-red-400' }}">
                        <span
                            class="w-2 h-2 rounded-full
                        {{ $reward->is_public ? 'bg-green-400' : 'bg-red-400' }}">
                        </span>
                ...
                <td class="px-6 py-4">
                    <div class="flex justify-center gap-2">
                        {{-- View --}}
                        <a href="{{ route('admin.rewards.show', $reward->id) }}" class="p-2 rounded-lg bg-blue-500/10 text-blue-400 hover:bg-blue-500/20">

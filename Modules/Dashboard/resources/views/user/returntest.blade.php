  <table id="table-data" class="w-full text-left bg-transparent border-collapse">
                    <thead class="bg-gray-50">
                        <tr class="text-sm border-b border-gray-200">
                            ...
                            <th class="px-6 py-4 kantumruy-pro font-medium text-gray-500">អ៊ីមែល</th>
                            <th class="px-6 py-4 kantumruy-pro font-medium text-gray-500">តួនាទី</th>
                            <th class="px-6 py-4 kantumruy-pro font-medium text-gray-500">ស្ថានភាព</th>
                        ...
                        @forelse ($users as $i => $user)
                            ...
                            <td class="px-6 py-4 kantumruy-pro">
                                <div class="flex items-center space-x-3">
                                    <div class="shrink-0">
                                        <img src="{{ $user->userDetail?->profile ? asset('profiles/' . $user->userDetail->profile) : asset('profiles/default_profile.jpg') }}" class="w-8 h-8 rounded-full object-cover border border-gray-200">
                                    </div>
                                    <div>
                                        <span class="font-medium text-gray-800">{{ $user->userDetail->first_name ?? ''  }}
                           ...
                           <td class="px-6 py-4 kantumruy-pro">
                            ...
                            <td class="px-6 py-4">
                                <div class="flex items-center space-x-2">
                                    <span class="w-2 h-2 rounded-full {{ $user->userDetail->is_active ? 'bg-green-500' : 'bg-red-500' }}"></span>
                                    <span class="text-sm text-gray-700">
                                        {{ $user->userDetail->is_active ? 'Active' : 'Blocked' }}
                                    </span>
                                </div>
                            </td>
